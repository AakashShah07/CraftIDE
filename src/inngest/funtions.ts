  ...
  async ({ event, step }) => {
    try {
      ...
      const codeAgent = createAgent(
        [terminalTool, createUpdateFile, readFile],
        {
      ...
      const response = await codeAgent.ask(
        `Write the following snippet: ${input}`
      );
      ...
      function extractTaskSummary(
        text: string | null | undefined
                ...
                console.log("Extract task summery ", text)
      ...
      const assistantText = lastAssistenttextMessageContent(response);

      const contentToSave = assistantText ?? "No response from DeepSeek";
      const summaryText = extractTaskSummary(assistantText);
